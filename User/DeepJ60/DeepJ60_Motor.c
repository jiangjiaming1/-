 ...
 * ---------------------------- Variable ----------------------------
 */
MotorInformation J60Motor;  // The information struct of J60-Motor
...
uint8_t SetMotionParameters (float Position, float Velocity, float Kp, float Kd, float Torque) {
    ...
    J60Motor.MotorCommand.MotorData.Position = Position;
    J60Motor.MotorCommand.MotorData.Velocity = Velocity;
    J60Motor.MotorCommand.MotorData.Kp = Kp;
    J60Motor.MotorCommand.MotorData.Kd = Kd;
    J60Motor.MotorCommand.MotorData.Torque = Torque;
...
uint8_t SetMotionCommand (uint8_t MotorID, uint8_t CommandID, float Position, float Velocity, float Kp, float Kd, float Torque) {
    J60Motor.MotorCommand.ID = CommandID;
    J60Motor.MotorCommand.MotorData.ID = MotorID;

    if (SetMotionParameters (Position, Velocity, Kp, Kd, Torque) == ERROR) {
        return ERROR;
    }
    return NORMAL;
}
...
void J60MotorDataToSendCanData (MotorInformation* Motor, CanFrame* Can) {
    if (Motor->MotorCommand.ID == CONTROL_MOTOR) {
        ...
        J60MotorConfigToSendCanData(&Motor->MotorConfig, Can->SetConfigSend);
    }
}
...
uint8_t DisableJ60Motor (uint8_t MotorID) {
    SetNormalCommand(MotorID, DISABLE_MOTOR);

    Can.ID = GetJ60MotorCanID(&J60Motor.MotorCommand);
...
uint8_t EnableJ60Motor (uint8_t MotorID) {
    SetNormalCommand(MotorID, ENABLE_MOTOR);
...
uint8_t GetJ60MotorConfig (uint8_t MotorID) {
    SetNormalCommand(MotorID, GET_CONFIG);
...
uint8_t ConfigJ60MotorCanTimeout (uint8_t MotorID, int CanTimeout) {
    SetNormalCommand(MotorID, SET_CAN_TIMEOUT);
    if (CanTimeout > CAN_TIMEOUT_MAX || CanTimeout < CAN_TIMEOUT_MIN) {
        return ERROR;
    }
    J60Motor.MotorConfig.CanTimeout = CanTimeout;

    Can.ID = GetJ60MotorCanID(&J60Motor.MotorCommand);
    Can.DLC = SEND_DLC_SET_CAN_TIMEOUT;
    J60MotorDataToSendCanData(&J60Motor, &Can);
...
uint8_t ConfigJ60MotorCurrentBandWidth (uint8_t MotorID, int CurrentBandWidth) {
    SetNormalCommand(MotorID, SET_BANDWIDTH);
    if (CurrentBandWidth > CURRENT_BAND_WIDTH_MAX || CurrentBandWidth < CURRENT_BAND_WIDTH_MIN) {
        return ERROR;
    }
    J60Motor.MotorConfig.CurrentBandWidth = CurrentBandWidth;
...
uint8_t SaveJ60MotorConfig (uint8_t MotorID) {
    SetNormalCommand(MotorID, SAVE_CONFIG);
...
uint8_t RunJ60MotorPositionMode (uint8_t MotorID, float Position, float Kp, float Kd) {
    if (SetMotionCommand(MotorID, CONTROL_MOTOR, Position, 0, Kp, Kd, 0) == ERROR) {
        return ERROR;
    }
...
uint8_t RunJ60MotorVelocityMode (uint8_t MotorID, float Velocity, float Kd) {
    if (SetMotionCommand(MotorID, CONTROL_MOTOR, 0, Velocity, 0, Kd, 0) == ERROR) {
        return ERROR;
    }
...
uint8_t RunJ60MotorTorqueMode (uint8_t MotorID, float Torque) {
    if (SetMotionCommand(MotorID, CONTROL_MOTOR, 0, 0, 0, 0, Toruqe) == ERROR) {
        return ERROR;
    }
...
uint8_t RunJ60MotorKdMode (uint8_t MotorID, float Kd) {
    if (SetMotionCommand(MotorID, CONTROL_MOTOR, 0, 0, 0, Kd, 0) == ERROR) {
        return ERROR;
    }
...
uint8_t RunJ60MotorZeroTorqueMode (uint8_t MotorID) {
    if (SetMotionCommand(MotorID, CONTROL_MOTOR, 0, 0, 0, 0, 0) == ERROR) {
        return ERROR;
    }
...
uint8_t RunJ60MotorPositionTorqueMode (uint8_t MotorID, float Position, float Torque, float Kp, float Kd) {
    if (SetMotionCommand(MotorID, CONTROL_MOTOR, Position, 0, Kp, Kd, Torque) == ERROR) {
        return ERROR;
    }
...
uint8_t RunJ60MotorVelocityTorqueMode (uint8_t MotorID, float Velocity, float Torque, float Kd) {
    if (SetMotionCommand(MotorID, CONTROL_MOTOR, 0, Velocity, 0, Kd, Torque) == ERROR) {
        return ERROR;
    }
...
uint8_t RunJ60MotorPositonVelocityTorqueMode (uint8_t MotorID, float Position, float Velocity, float Torque, float Kp, float Kd) {
    if (SetMotionCommand(MotorID, CONTROL_MOTOR, Position, Velocity, Kp, Kd, Torque) == ERROR) {
        return ERROR;
    }
...
uint8_t RunJ60Motor (uint8_t MotorID, float Position, float Velocity, float Torque, float Kp, float Kd, enum MotorMode Mode) {
    ...
    J60Motor.MotorMode = Mode;

    switch (Mode) {
        case PositionMode: 
            if (RunJ60MotorPositionMode(MotorID, Position, Kp, Kd) == ERROR) {
                return ERROR;
            }
            break;

        case VelocityMode:
            if (RunJ60MotorVelocityMode(MotorID, Velocity, Kd) == ERROR) {
                return ERROR;
            }
            break;

        case TorqueMode:
            if (RunJ60MotorTorqueMode(MotorID, Torque) == ERROR) {
                return ERROR;
            }
            break;

        case KdMode: 
            if (RunJ60MotorKdMode(MotorID, Kd) == ERROR) {
                return ERROR;
            }
            break;

        case ZeroTorqueMode: 
            if (RunJ60MotorZeroTorqueMode(MotorID) == ERROR) {
                return ERROR;
            }
            break;

        case PositionTorqueMode: 
            if (RunJ60MotorPositionTorqueMode(MotorID, Position, Torque, Kp, Kd) == ERROR) {
                return ERROR;
            }
            break;
        
        case VelocityTorqueMode: 
            if (RunJ60MotorVelocityTorqueMode(MotorID, Velocity, Torque, Kd) == ERROR) {
                return ERROR;
            }
            break;

        case PositionVelocityTorqueMode:
            if (RunJ60MotorPositonVelocityTorqueMode(MotorID, Position, Velocity, Torque, Kp, Kd) == ERROR) {
                return ERROR;
            }
            break;
...
uint8_t GetJ60MotorStatusWord (uint8_t MotorID) {
    SetNormalCommand(MotorID, GET_STATUS_WORD);
...
uint8_t J60MotorErrorReset (uint8_t MotorID) {
    SetNormalCommand(MotorID, ERROR_RESET);
...
void AnalyseJ60MotorReceiveData (void) {
    ReceiveCanDataToJ60MotorData(&J60Motor, &Can);
}
